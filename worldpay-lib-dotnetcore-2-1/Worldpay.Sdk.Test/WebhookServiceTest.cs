...
namespace Worldpay.Sdk.Test
{
    ...
    public class WebhookServiceTest
    {
        ...
        public void shouldLogOrderStatus()
        {
            ...
            var serializedData = JsonConvert.SerializeObject(notification);
            ...
            using (var reader = new StreamReader(new FileStream(Configuration.OrderLog, FileMode.Open)))
            {
                ...
                //Assert.AreEqual($"{notificationEventType}: {orderCode} {status.ToString()}", firstLine);
                Assert.Equal($"{notificationEventType}: {orderCode} {status.ToString()}", firstLine);
